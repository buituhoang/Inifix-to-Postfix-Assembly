.data
	askMessage: .asciiz "Do you want to continue"
	endMessage: .asciiz "Goodbye. Thank you for using me"
	...
	startMessage: .asciiz "Enter infix expression\nNote: only allowed to use + - * / ()\nNumber from 00-99"
...
# Check all valid input option
	...
	continueScan:
	beq $t4, 's', squareOperator
	beq $t4, 'S', squareOperator
	beq $t4, '+', plusMinusOperator
	beq $t4, '-', plusMinusOperator
	beq $t4, '*', multiplyDivideOperator
	beq $t4, '/', multiplyDivideOperator
	beq $t4, '(', openBracket
	beq $t4, ')', closeBracket
...
squareOperator:				# Input is square
	li $s7,5			# Change input status to 5
	beq $t5,-1,addMore
	continueSquare:
...
plusMinusOperator:			# Input is + -
	...
	beq $t7,'s',lowerPrecedence
multiplyDivideOperator:			# Input is * /
	beq $s7,2,wrongInput		# Receive operator after operator or open bracket
	beq $s7,3,wrongInput
	beq $s7,0,wrongInput		# Receive operator before any number
	li $s7,2			# Change input status to 2
